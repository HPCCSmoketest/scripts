...
then
    ...
    cat prp-${day}.log | egrep '^([0-9]*)/([0-9]*)\. Process|wait|^(\s*)sha|^(\s*)base|^(\s*)user|HPCC Star|^(\s*)start|^(\s*)end|^(\s*)pass|scheduled|done, exit' | egrep -i -B7 'pass : True' | egrep 'PR-' | sed -n 's/^\(.*\)\s\(PR-[0-9].*\),\(.*\)$/    \2/p' | sort -u
else
